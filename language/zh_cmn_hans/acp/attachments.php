...
$lang = array_merge($lang, array(
	'ACP_ATTACHMENT_SETTINGS_EXPLAIN'	=> '这里您可以配置附件和其分类的设定。',
	'ACP_EXTENSION_GROUPS_EXPLAIN'		=> '这里您可以添加，删除，修改或禁用扩展名类别。选项包括设定附件类别，更改下载机制和定义附件组的简明图标。',
	...
	'ACP_ORPHAN_ATTACHMENTS_EXPLAIN'	=> '这里您可以看见附件上传目录中存在但是没有发表在帖子中的文件。这很可能因为用户添加了附件但是最后没提交帖子。您可以删除这些附件或者将其添加到已有的帖子中。添加附件到帖子中需要一个有效的帖子 ID，您需要自己添加这个 ID，这个功能主要是帮助那些希望通过其他方式上传文件的人们，可以把附件（通常会很大）添加到已经发表的帖子中。',
	'ADD_EXTENSION'						=> '添加扩展名',
	'ADD_EXTENSION_GROUP'				=> '添加扩展名类别',
	...
	'ASSIGNED_GROUP'					=> '指定的扩展名类别',
	'ATTACH_EXTENSIONS_URL'				=> '扩展名',
	'ATTACH_EXT_GROUPS_URL'				=> '扩展名类别',
	'ATTACH_ID'							=> 'ID',
	'ATTACH_MAX_FILESIZE'				=> '最大文件尺寸',
	'ATTACH_MAX_FILESIZE_EXPLAIN'		=> '单个文件的最大尺寸，如果设为 0 的话上传文件大小只受限于您的 PHP 配置。',
	'ATTACH_MAX_PM_FILESIZE'			=> '最大短信文件尺寸',
	'ATTACH_MAX_PM_FILESIZE_EXPLAIN'	=> '单个用户短信附件的文件大小上限， 0 表示无限制。',
	'ATTACH_ORPHAN_URL'					=> '幽灵附件文件',
	'ATTACH_POST_ID'					=> '帖子 ID',
	'ATTACH_POST_TYPE'					=> '帖子类型',
	'ATTACH_QUOTA'						=> '合计附件空间',
	'ATTACH_QUOTA_EXPLAIN'				=> '整个论坛可用的最大附件空间， 0 表示无限制。',
	'ATTACH_TO_POST'					=> '添加附件到帖子',

	'CAT_IMAGES'				=> '图片',
	'CHECK_CONTENT'				=> '检查附件',
	'CHECK_CONTENT_EXPLAIN'		=> '一些浏览器可能被欺骗而得到错误的附件文件类型。这个选项可以确保这样的文件不会被上传。',
	'CREATE_GROUP'				=> '创建新类别',
	'CREATE_THUMBNAIL'			=> '创建缩略图',
	'CREATE_THUMBNAIL_EXPLAIN'	=> '在所有可能的情况下创建缩略图。',

	'DEFINE_ALLOWED_IPS'			=> '设置允许的 IP 地址/主机名',
	'DEFINE_DISALLOWED_IPS'			=> '设置封禁的 IP 地址/主机名',
	'DOWNLOAD_ADD_IPS_EXPLAIN'		=> '设置多个 IP 或主机名， 请使用多行输入。设置 IP 地址范围，请在起止间用破折号 (-)，通配符请用 * 号',
	...
	'EXCLUDE_ENTERED_IP'			=> '选中这个以排除输入的 IP 地址/主机名。',
	'EXCLUDE_FROM_ALLOWED_IP'		=> '从允许的 IP 地址/主机名中排除 IP',
	'EXCLUDE_FROM_DISALLOWED_IP'	=> '从封禁的 IP 地址/主机名中排除 IP',
	...
	'FILES_GONE'			=> '一些您要删除的附件不存在，它们可能已被删除，曾经存在的附件已被删除。',
	'FILES_STATS_WRONG'		=> '您的文件统计数据好像不准确，需要重新同步，实际值：附件数量 = %1$d，附件总大小 = %2$s。<br />点%3$s这里%4$s重新同步它们。',
	...
	'IMAGE_QUALITY'				=> '上传的图片附件质量（仅 JPEG 文件）',
	'IMAGE_QUALITY_EXPLAIN'		=> '指定值范围 50% （较小文件） 到 90% (较高画质)。超过90% 会增加文件的大小，所以不能超过 90%。只有当最大图片尺寸设置不是 0px × 0px 时，此设置生效。',
	'IMAGE_STRIP_METADATA'		=> '清除图片的元数据（仅JPEG 文件）',
	'IMAGE_STRIP_METADATA_EXPLAIN'	=> '清除图片的 Exif 元数据，比如作者名字，GPS 定位信息，照相机参数等。 只有当最大图片尺寸设置不是 0px × 0px 时，此设置生效。',
	...
	'MAX_IMAGE_SIZE_EXPLAIN'		=> '检查图片附件的最大尺寸。phpBB可能会在上传前尝试自动调整图片大小，使其符合这些最大尺寸要求。将值都设为 0px 可禁用尺寸检查和自动调整大小的功能。',
	'MAX_THUMB_WIDTH'				=> '缩略图的最大宽度/高度(像素值)',
	...
	'NOT_ASSIGNED'				=> '没有指定',
	'NO_ATTACHMENTS'			=> '没有找到这段时间的附件。',
	'NO_EXT_GROUP'				=> '无',
	'NO_EXT_GROUP_ALLOWED_PM'	=> '对私信，没有 <a href="%s">允许的扩展组</a>。',
	'NO_EXT_GROUP_ALLOWED_POST'	=> '对帖子，没有 <a href="%s">允许的扩展组</a> for posts。',
	'NO_EXT_GROUP_NAME'			=> '没有输入类别名称',
	'NO_EXT_GROUP_SPECIFIED'	=> '没有指定扩展名类别。',
	'NO_FILE_CAT'				=> '无',
	'NO_IMAGE'					=> '没有图片',
	'NO_UPLOAD_DIR'				=> '您指定的上传目录不存在。',
	'NO_WRITE_UPLOAD'			=> '您指定的上传目录不可写入。请更改目录权限使 web 服务器可以作写操作。',

	'ONLY_ALLOWED_IN_PM'	=> '只在站内短信中允许',
	'ORDER_ALLOW_DENY'		=> '允许',
	'ORDER_DENY_ALLOW'		=> '禁止',

	'REMOVE_ALLOWED_IPS'		=> '删除或排除 <em>允许</em> 的 IP 地址/主机名',
	'REMOVE_DISALLOWED_IPS'		=> '删除或排除 <em>禁止</em> 的 IP 地址/主机名',
	'RESYNC_FILES_STATS_CONFIRM'	=> '您确定要同步文件状态吗？',

	'SECURE_ALLOW_DENY'				=> '允许/封禁 列表',
	'SECURE_ALLOW_DENY_EXPLAIN'		=> '当防盗链启用后， <strong>允许列表</strong> 或 <strong>封禁列表</strong> 规定了特例的情况。',
	'SECURE_DOWNLOADS'				=> '启用防盗链功能',
	'SECURE_DOWNLOADS_EXPLAIN'		=> '当开启选项后，下载将仅对您自己的域名内的链接和下面设定的 IP 地址/主机名有效。',
	...
	'UPLOADING_FILE_TO'				=> '上传文件 “%1$s” 到帖子ID %2$d…',
	'UPLOAD_DENIED_FORUM'			=> '您没有上传文件到版面 “%s” 的权限',
	'UPLOAD_DIR'					=> '上传目录',
	'UPLOAD_DIR_EXPLAIN'			=> '附件的存储路径。',
	'UPLOAD_ICON'					=> '上传图标',
	'UPLOAD_NOT_DIR'				=> '您指定的上传地址不是一个目录。',
	'UPLOAD_POST_NOT_EXIST'			=> '文件 “%1$s” 无法上传到对应的帖子 %2$d ，因为其不存在。',
));
