...
$lang = array_merge($lang, array(
	'ACP_BBCODES_EXPLAIN'		=> 'BBCode 是一种 HTML 的特殊实现，可以产生各种页面效果而不会有安全隐患。这里您可以添加/删除/编辑自定义 BBCode。',
	'ADD_BBCODE'				=> '添加 BBCode',

	'BBCODE_DANGER'				=> '您添加的 BBCode 看上去不太安全。如果在敏感内容中使用了 {TEXT} 标记，请使用更安全的类型。只有在您明白此设置的风险后才继续。',
	'BBCODE_DANGER_PROCEED'		=> '我明白，继续', //'I understand the risk',

	'BBCODE_ADDED'				=> 'BBCode 添加完成。',
	'BBCODE_EDITED'				=> 'BBCode 编辑完成。',
	'BBCODE_DELETED'			=> 'BBCode 已成功移除。',
	'BBCODE_NOT_EXIST'			=> '您选择的 BBCode 不存在。',
	'BBCODE_HELPLINE'			=> '提示',
	'BBCODE_HELPLINE_EXPLAIN'	=> '这个区域会在鼠标移过时显示 BBCode 提示信息',
	'BBCODE_HELPLINE_TEXT'		=> '提示文本',
	'BBCODE_HELPLINE_TOO_LONG'   => '您设定的提示文本过长。',

	'BBCODE_INVALID_TAG_NAME'	=> '您选择的 BBCode 标签名称已经存在。',
	'BBCODE_INVALID'			=> '您构建的 BBCode 形式无效。',
	'BBCODE_INVALID_TEMPLATE'	=> '您的 BBCode 模板无效。',
	'BBCODE_TAG'				=> '标签',
	'BBCODE_TAG_TOO_LONG'		=> '您选择的标签名称太长了。',
	'BBCODE_TAG_DEF_TOO_LONG'	=> '您输入的标签定义太长了，请尽量简短。',
	'BBCODE_USAGE'				=> 'BBCode 使用方法',
	'BBCODE_USAGE_EXAMPLE'		=> '[highlight={COLOR}]{TEXT}[/highlight]<br /><br />[font={SIMPLETEXT1}]{SIMPLETEXT2}[/font]',
	'BBCODE_USAGE_EXPLAIN'		=> '这里您可以定义 BBcode 的用法。使用相关的标记替换相关的变量输入(%s如下所示%s)',

	'EXAMPLE'						=> '例子：',
	'EXAMPLES'						=> '例子：',

	'HTML_REPLACEMENT'				=> 'HTML 替换代码',
	'HTML_REPLACEMENT_EXAMPLE'		=> '&lt;span style="background-color: {COLOR};"&gt;{TEXT}&lt;/span&gt;<br /><br />&lt;span style="font-family: {SIMPLETEXT1};"&gt;{SIMPLETEXT2}&lt;/span&gt;',
	'HTML_REPLACEMENT_EXPLAIN'		=> '这里您可以定义默认的 HTML 替换代码 (每个模板可以使用它自己的替换代码)。不要忘记放回您上面使用的标记!',

	'TOKEN'					=> '标记',
	'TOKENS'				=> '标记',
	'TOKENS_EXPLAIN'		=> '标记是用户输入的占位符。只有满足设定的输入才能通过验证。如果需要的话，您可以使用数字对它们进行编号，例如 {TEXT1}，{TEXT2}.<br /><br />在HTML替换语句中您可以使用 language 目录下的任意语言条目，例如: {L_<em>&lt;STRINGNAME&gt;</em>} 这里 <em>&lt;STRINGNAME&gt;</em> 就是语言包下的一个翻译条目。例如，{L_WROTE} 在英文中将被显示为 &quot;wrote&quot; 或者用户所使用的语言的翻译.<br /><br /><strong>请注意只有如下列出的标记可以在自定义BBCode中使用.</strong>',
	'TOKEN_DEFINITION'		=> '这将是？',
	'TOO_MANY_BBCODES'		=> '您不能创建更多 BBCodes。请删除掉一些现有的BBCode后再重试。',

	'tokens'	=>	array(
		'TEXT'			=> '任何文本，包括外文字符，数字等等..。',
		'SIMPLETEXT'	=> '拉丁字符 (A-Z)，数字，空格，逗号，点，减号，加号，连字号和下划线',
		'INTTEXT'		=> 'Unicode字符，数字，空格，逗号，点，减号，加号，连字符和下划线。',
		'IDENTIFIER'	=> '拉丁字符 (A-Z)，数字，连字符和下划线',
		'NUMBER'		=> '任何数字序列',
		'EMAIL'			=> '有效的 email 地址',
		'URL'			=> '有效的 URL 须使用任何允许的协议(http，ftp，等等… 不能用作 javascript 刺探)。如果没有提供，将添加“http://”作为前缀。',
		'LOCAL_URL'		=> '本地 URL。 URL 必须和主题页相关并且不能包含服务器名称和协议，就像带前缀"%s"的链接',
		'RELATIVE_URL'	=> '相对 URL。您可以使用这个来匹配部分 URL，但是注意： 一个完整的 URL 是一个有效的相对 URL。当您想使用相对 URL 时,使用 LOCAL_URL 标记。',
		'COLOR'			=> 'HTML 颜色，可以是六位十六进制数<samp>#FF1234</samp> 或者<a href="http://www.w3.org/TR/CSS21/syndata.html#value-def-color">CSS色彩关键字</a> 例如<samp>fuchsia</samp> 或 <samp>InactiveBorder</samp>',
		'ALNUM'			=> '拉丁文字母（A-Z）和数字的字符。',
		'CHOICE'		=> '特殊值的选择，比如 <samp>{CHOICE=spades,hearts,diamonds,clubs}</samp>。 此值默认大小写不敏感，也可以指定 <samp>caseSensitive</samp> 选项使之大小写敏感 <samp>{CHOICE=Spades,Hearts,Diamonds,Clubs;caseSensitive}</samp> 。',
		'FLOAT'			=> '十进制数值，例如 <samp>0.5</samp>。',
		'HASHMAP'		=> '以 <samp>{HASHMAP=string1:replacement1,string2:replacement2}</samp> 的形式将字符串映射到其替换字符串。区分大小写。默认情况下保留未知值。',
		'INT'			=> '整数型数值，例如 <samp>2</samp>。',
		'IP'			=> '一个有效的 IPv4 或 IPv6 地址。',
		'IPPORT'		=> '一个带有端口值的有效 IPv4 或 IPv6 地址。',
		'IPV4'			=> '一个有效的 IPv4 地址。',
		'IPV6'			=> '一个有效的 IPv6 地址。',
		'MAP'			=> '以 <samp>{MAP=string1:replacement1,string2:replacement2}</samp> 的形式将字符串映射到其替换字符串。不区分大小写。默认情况下保留未知值。',
		'RANGE'			=> '接受给定范围内的整数值，例如 <samp>{RANGE=-10,42}</samp>。',
		'REGEXP'		=> '用指定的正则表达式来校验输入，例如 <samp>{REGEXP=/^foo\w+bar$/}</samp>。',
		'TIMESTAMP'		=> '时间戳，如 <samp>1h30m10s</samp>，将转换为秒数。也接受数字。',
		'UINT'			=> '无符号整数值。与 <samp>{INT}</samp>相同，但拒绝小于 0 的值。',
...
$lang = array_merge($lang, array(
	...
	'ACP_SMILIES_EXPLAIN'	=> '表情图标一般是较小的图片，有时也可以是动画，是用于表达情绪的好方式。这里您可以添加/删除/编辑表情图标。您也可以安装和创建新的表情图片包。',
	...
	'EXPORT_SMILIES'			=> '导出并下载 smilies.pak',
...
// Word censors
$lang = array_merge($lang, array(
	'ACP_WORDS_EXPLAIN'		=> '这个控制面板中可以添加，编辑，删除需要过滤的词汇。但是用户将仍可以使用包含这些敏感词的用户名注册帐号。可以使用通配符，例如 *test* 将匹配 detestable，test* 将匹配 testing，*test 将匹配 detest',
	'ADD_WORD'				=> '添加敏感词',
...
$lang = array_merge($lang, array(
	...
	'IS_TRANSLATED_EXPLAIN'		=> '原因已经被翻译成本地语言。如果您您输入的标题在语言文件的举报原因章节已经设置过，本地化的标题和描述将自动启用。',
