<?php

if (!defined('IN_PHPBB'))
{
	exit;
}

if (empty($lang) || !is_array($lang))
{
	$lang = [];
}

$lang = array_merge($lang, [
	'ACL_U_TOPTOPICS_DISLIKE' => 'Top Topics: Can dislike posts',
	'TOPTOPICS_INDEX_TITLE' => 'Top Topics',
	'TOPTOPICS_FORUM_TITLE' => 'Top Topics In This Forum',
	'TOPTOPICS_SCORE' => 'Score',
	'TOPTOPICS_SIGNALS' => 'Signals',
	'TOPTOPICS_LIKES' => 'Likes',
	'TOPTOPICS_DISLIKES' => 'Dislikes',
	'TOPTOPICS_VIEWS' => 'Views',
	'TOPTOPICS_FLAGS' => 'Flags',
	'TOPTOPICS_REACTIONS' => 'Reactions',
	'TOPTOPICS_REACTION_RECORDS' => 'Received likes/dislikes',
	'TOPTOPICS_REACTION_RECORDS_VIEW' => 'View received likes/dislikes',
	'TOPTOPICS_REACTION_RECORDS_EXPLAIN' => 'This page shows likes and dislikes received by your posts. Only records in forums you can read are shown.',
	'TOPTOPICS_NO_REACTION_RECORDS' => 'No received likes or dislikes yet.',
	'TOPTOPICS_REACTION_LIKE' => 'liked',
	'TOPTOPICS_REACTION_DISLIKE' => 'disliked',
	'TOPTOPICS_RECEIVED_REACTION_LINE' => '%1$s - %2$s <b>%3$s</b> your post "%4$s" in topic "%5$s"',
	'TOPTOPICS_POINTS' => 'Points',
	'TOPTOPICS_REPUTATION' => 'Reputation',
	'TOPTOPICS_REPUTATION_TIER_NEGATIVE' => 'Foggy Pen',
	'TOPTOPICS_REPUTATION_TIER_NEUTRAL' => 'Fresh Ink',
	'TOPTOPICS_REPUTATION_TIER_POSITIVE' => 'Steady Voice',
	'TOPTOPICS_REPUTATION_TIER_TRUSTED' => 'Signal Maker',
	'TOPTOPICS_REPUTATION_TIER_ELITE' => 'Forum Pillar',
	'TOPTOPICS_REPUTATION_TIER_LEGEND' => 'Legendary Pen',
	'TOPTOPICS_REPUTATION_TOOLTIP' => 'Reputation: %1$s - %2$s. Useful posts with direct positive feedback increase reputation.',
	'TOPTOPICS_TOPIC_MIN_REPUTATION' => 'Minimum reputation to view',
	'TOPTOPICS_TOPIC_MIN_REPUTATION_EXPLAIN' => 'Use 0 for everyone. A positive value keeps the title visible but protects the topic body from users below that reputation. Your current reputation is %1$s.',
	'TOPTOPICS_TOPIC_MIN_REPUTATION_INVALID' => 'The minimum reputation must be a whole number between 0 and 1,000,000.',
	'TOPTOPICS_TOPIC_REPUTATION_REQUIRED' => 'This topic requires reputation %1$s or higher. Your current reputation is %2$s.',
	'TOPTOPICS_TOPIC_REPUTATION_BADGE' => 'Reputation %1$s+',
	'TOPTOPICS_TOPIC_REPUTATION_BADGE_EXPLAIN' => 'The topic title is public, but reading the body requires reputation %1$s or higher.',
	'TOPTOPICS_REPUTATION_TOPIC_CANNOT_COPY_TO_BLOG' => 'A reputation-protected topic cannot also be copied to the public blog. Use Submit post, or set the minimum reputation to 0.',
	'TOPTOPICS_CATEGORY_EMPTY' => 'No top topics in this category yet',
	'TOPTOPICS_CATEGORY_MORE' => 'More...',
	'TOPTOPICS_RANK_TITLE' => 'Ranked by decayed score using likes, dislikes, non-author replies, views, report penalties, and discussion balance',
	'TOPTOPICS_ADMIN_OVERRIDE' => 'Top Topics admin override',
	'TOPTOPICS_OVERRIDE_NORMAL' => 'Normal',
	'TOPTOPICS_OVERRIDE_BOOST' => 'Boost',
	'TOPTOPICS_OVERRIDE_DEMOTE' => 'Demote',
	'TOPTOPICS_OVERRIDE_KILL' => 'Kill',
	'TOPTOPICS_OVERRIDE_CURRENT_NORMAL' => 'Current state: normal',
	'TOPTOPICS_OVERRIDE_CURRENT_BOOST' => 'Current state: boosted',
	'TOPTOPICS_OVERRIDE_CURRENT_DEMOTE' => 'Current state: demoted',
	'TOPTOPICS_OVERRIDE_CURRENT_KILL' => 'Current state: hidden',
	'TOPTOPICS_COLLAPSE_SHOW' => 'Show top topics',
	'TOPTOPICS_COLLAPSE_HIDE' => 'Hide top topics',
	'TOPTOPICS_SHOW_FORUM_SUMMARY' => 'Show Top Topics on forum pages',
	'TOPTOPICS_SHOW_FORUM_SUMMARY_EXPLAIN' => 'Allow the Top Topics list to appear on individual forum pages.',
	'TOPTOPICS_SHOW_ENHANCED_TOPIC_LIST_VIEW' => 'Show enhanced topic list view',
	'TOPTOPICS_SHOW_ENHANCED_TOPIC_LIST_VIEW_EXPLAIN' => 'The default is the classic forum layout. Enable this to show inline previews in topic lists.',
	'TOPTOPICS_SHOW_FLAT_TOPIC_LIST_VIEW' => 'Use merged new-posts homepage',
	'TOPTOPICS_SHOW_FLAT_TOPIC_LIST_VIEW_EXPLAIN' => 'When enabled, the homepage uses a merged sitewide new-posts list. When disabled, the homepage uses forum sections. This does not change the topic-list display format.',
	'TOPTOPICS_SHOW_TOPIC_LIST_MEDIA_PREVIEWS' => 'Show media previews in topic lists',
	'TOPTOPICS_SHOW_TOPIC_LIST_MEDIA_PREVIEWS_EXPLAIN' => 'When disabled, text excerpts still appear, but image, video, tweet, and other media previews are hidden.',
	'TOPTOPICS_HEADER_THEME_TOGGLE' => 'Switch style',
	'TOPTOPICS_HEADER_THEME_GOLD' => 'Gold style',
	'TOPTOPICS_HEADER_THEME_GOLD_EXPLAIN' => 'Switch to the gold style',
	'TOPTOPICS_HEADER_THEME_COOL' => 'Blue style',
	'TOPTOPICS_HEADER_THEME_COOL_EXPLAIN' => 'Switch to the blue style',
	'TOPTOPICS_HEADER_THEME_GRAY' => 'Gray style',
	'TOPTOPICS_HEADER_THEME_GRAY_EXPLAIN' => 'Switch to the gray style',
	'TOPTOPICS_HEADER_TOPIC_LIST_TOGGLE' => 'Switch topic list',
	'TOPTOPICS_HEADER_TOPIC_LIST_CLASSIC' => 'No-image list',
	'TOPTOPICS_HEADER_TOPIC_LIST_CLASSIC_EXPLAIN' => 'Classic topic list without preview images',
	'TOPTOPICS_HEADER_TOPIC_LIST_ENHANCED' => 'Image list',
	'TOPTOPICS_HEADER_TOPIC_LIST_ENHANCED_EXPLAIN' => 'Enhanced topic list with preview images',
	'TOPTOPICS_HEADER_TOPIC_LIST_FLAT' => 'No forums',
	'TOPTOPICS_HEADER_TOPIC_LIST_FLAT_EXPLAIN' => 'Show only a sitewide new-posts list on the homepage, without forum sections',
	'TOPTOPICS_HEADER_TOPIC_LIST_COMPACT' => 'Compact',
	'TOPTOPICS_HEADER_TOPIC_LIST_COMPACT_EXPLAIN' => 'Two-column title list without excerpts, stats, or latest-post columns',
	'TOPTOPICS_HEADER_HOME_LAYOUT_TOGGLE' => 'Switch homepage layout',
	'TOPTOPICS_HEADER_HOME_LAYOUT_SPLIT' => 'Sections',
	'TOPTOPICS_HEADER_HOME_LAYOUT_SPLIT_EXPLAIN' => 'Show homepage topics grouped by forum sections',
	'TOPTOPICS_HEADER_HOME_LAYOUT_MERGED' => 'Merged',
	'TOPTOPICS_HEADER_HOME_LAYOUT_MERGED_EXPLAIN' => 'Show a merged sitewide new-posts list on the homepage',
	'TOPTOPICS_FLAT_EMPTY' => 'No new posts.',
	'TOPTOPICS_FLAT_TOTAL' => [
		0 => '%d new posts',
		1 => '%d new post',
		2 => '%d new posts',
	],
	'TOPTOPICS_FLAT_POST_TOPIC_BUTTON' => 'Choose forum to post a topic',
	'TOPTOPICS_NEW_MEMBER_APPROVAL_NOTICE' => [
		1 => 'Newly registered users need %1$d approved post before posts stop entering the new-user approval queue. You currently have %2$d, with %3$d more needed. This post will not be visible to others until approved.',
		2 => 'Newly registered users need %1$d approved posts before posts stop entering the new-user approval queue. You currently have %2$d, with %3$d more needed. This post will not be visible to others until approved.',
	],
	'TOPTOPICS_HOME_FORUM_EXCLUSIONS' => 'Hide forums from homepage topic lists',
	'TOPTOPICS_HOME_FORUM_EXCLUSIONS_EXPLAIN' => 'Selected forums will not appear in the homepage Recent Topics or Top Topics lists.',
	'TOPTOPICS_FROM_COLUMN' => 'From',
	'TOPTOPICS_TOPIC_LABEL' => 'Topic',
	'TOPTOPICS_FORUM_LABEL' => 'Forum',
	'TOPTOPICS_LATEST' => 'Latest',
	'TOPTOPICS_POST_COLLAPSED' => 'This post is collapsed because dislikes exceed likes by %1$d.',
	'CLICK_TO_DISLIKE' => 'click to dislike this post',
	'CLICK_TO_UNDISLIKE' => 'click to remove your dislike from this post',
	'LOGIN_TO_DISLIKE_POST' => 'login to dislike this post',
	'CANT_DISLIKE_OWN_POST' => 'sorry, you cannot dislike your own post',
	'TOPTOPICS_MIN_POSTS_TO_DISLIKE' => 'you need at least %d posts before you can dislike content',
	'TOPTOPICS_MIN_REPUTATION_TO_DISLIKE' => 'you need reputation %1$s or higher to dislike content. Your current reputation is %2$s',
	'TOPTOPICS_MIN_REPUTATION_TO_REPORT' => 'you need reputation %1$s or higher to report content. Your current reputation is %2$s',
	'TOPTOPICS_RATE_LIMIT_MINUTE' => 'you have reached the downvote rate limit for this minute',
	'TOPTOPICS_RATE_LIMIT_DAY' => 'you have reached the downvote rate limit for today',
	'TOPTOPICS_REMOVE_LIKE_FIRST' => 'remove your like first',
	'TOPTOPICS_REMOVE_DISLIKE_FIRST' => 'remove your dislike first',
	'TOPTOPICS_DISLIKED_BY' => 'post disliked by: ',
	'TOPTOPICS_DISLIKES_COUNT' => [
		0 => '%d dislikes',
		1 => '%d dislike',
		2 => '%d dislikes',
	],
]);
