...
/**
 *
 * Freemit BBS Blog. An extension for the phpBB Forum Software package.
 *
 */
...
{
	private const BLOG_FORUM_NAME = '坛友博客';
	private const BLOG_FORUM_DESC = '坛友发表博客文章的地方。';

	public static function depends_on()
	{
		return ['\freemitbbs\blog\migrations\release_1_0_2'];
	}

	public function update_data()
	{
		return [
			['custom', [[$this, 'ensure_hidden_blog_forum']]],
		];
	}

	public function ensure_hidden_blog_forum(): void
	{
		$forum_id = $this->get_configured_forum_id();

		if ($forum_id > 0 && $this->forum_exists($forum_id) && !$this->is_managed_blog_forum($forum_id))
		{
			$forum_id = 0;
		}
		...
		{
			$forum_id = $this->create_blog_forum();
		}

		$this->normalise_blog_forum($forum_id);
		$this->config->set('freemitbbs_blog_forum_id', (string) $forum_id);
		$this->config->set('freemitbbs_blog_forum_managed', (string) $forum_id);
		$this->set_blog_forum_permissions($forum_id);
		$this->clear_acl_cache();
	}

	protected function get_configured_forum_id(): int
	{
		return (int) $this->get_config_value('freemitbbs_blog_forum_id');
	}
	...
	protected function is_managed_blog_forum(int $forum_id): bool
	{
		return $forum_id > 0
			&& (int) $this->get_config_value('freemitbbs_blog_forum_managed') === $forum_id;
	}
	...
	protected function create_blog_forum(): int
	{
		...
		$sql_ary = [
			...
			'right_id' => $right_id + 2,
			'forum_parents' => '',
			'forum_name' => self::BLOG_FORUM_NAME,
			'forum_desc' => self::BLOG_FORUM_DESC,
			'forum_desc_bitfield' => '',
	...
	protected function normalise_blog_forum(int $forum_id): void
	{
	...
	protected function set_blog_forum_permissions(int $forum_id): void
	{
		...
		$this->add_group_role($role_rows, $groups, $roles, 'REGISTERED', ['ROLE_FORUM_POLLS', 'ROLE_FORUM_STANDARD'], $forum_id);
		$this->add_group_role($role_rows, $groups, $roles, 'REGISTERED_COPPA', ['ROLE_FORUM_POLLS', 'ROLE_FORUM_STANDARD'], $forum_id);
		$this->add_group_role($role_rows, $groups, $roles, 'GLOBAL_MODERATORS', ['ROLE_FORUM_POLLS', 'ROLE_FORUM_STANDARD'], $forum_id);
		...
		$this->add_group_role($role_rows, $groups, $roles, 'NEWLY_REGISTERED', ['ROLE_FORUM_NEW_MEMBER', 'ROLE_FORUM_STANDARD'], $forum_id);
